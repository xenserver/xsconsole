...
class SRNewDialogue(Dialogue):
    ...
    def BuildPanePROBE_NETAPP_PROVISIONING(self):
        ...
        self.provisioningMenu.AddChoice(name = Lang('Thick Provisioning'),
            onAction = self.HandleProvisioningChoice,
            handle = 'THICK')

        self.provisioningMenu.AddChoice(name = Lang('Thin Provisioning Without A-SIS Deduplication'),
            onAction = self.HandleProvisioningChoice,
            handle = 'THIN_NO_ASIS')

        if self.srParams['aggregate'].asisdedup.lower().startswith('true'):
            self.provisioningMenu.AddChoice(name = Lang('Thin Provisioning With A-SIS Deduplication'),
                onAction = self.HandleProvisioningChoice,
                handle = 'THIN_ASIS')
        else:
    ...
    def UpdateFieldsGATHER_NETAPP(self):
        ...
        pane.AddInputField(Lang('Description', 26), '', 'description')
        pane.AddInputField(Lang('NetApp Filer Address', 26), 'devfiler', 'target')
        pane.AddInputField(Lang('Username', 26), 'root', 'username')
        pane.AddPasswordField(Lang('Password', 26), 'xenroot', 'password')
        pane.AddInputField(Lang('CHAP Username', 26), '', 'chapusername')
    ...
    def NetAppBaseConfig(self):
        ...
        if self.srParams['chapusername'] != '':
            retVal.update({
                'chapusername':self.srParams['chapusername'], # FIXME: test these
    ...
    def HandleNetAppData(self, inParams):
        ...
        self.extraInfo = [ # Array of tuples
            ...
            (Lang('Username'), self.srParams['username']),
            (Lang('Password'), '*' * len(self.srParams['password'])),
            (Lang('CHAP Username'), self.srParams['chapusername']),
    ...
    def HandleProvisioningChoice(self, inChoice):
        ...
        self.extraInfo.append( (Lang('provisioning'), inChoice) ) # Append tuple, so double brackets
        self.ChangeState('CONFIRM')
